...
| サービス | 役割 |
| --- | --- |
| `db` | PostgreSQL。レース、出走馬、過去戦績、予測、買い目、ジョブ履歴、ジョブ予約、設定を保存します。 |
...
買い目判定では、単勝・複勝・馬連・ワイドの候補を作り、予測確率とオッズから期待値を計算します。単勝はモデルの 1 着確率、複勝・馬連・ワイドはその確率からの近似を使います。Web UI では券種別回収率、オッズ取得状況、買い目候補を確認できます。
